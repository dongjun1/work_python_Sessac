...
if __name__ == '__main__':
    ...
    # (simple_train, simple_valid, simple_test), simple_source_vocab, simple_source_vocab = simple_samples(path, device, batch_size = batch_size)


    # for k, v in target_vocab.idx2word.items():
    #     print(k, v)
    attention = 'LuongAttention'
    encoder = Encoder(device, source_vocab, embbeding_dim, hidden_dim, encoder_model).to(device)
    decoder = Decoder(device, target_vocab, embbeding_dim, hidden_dim, decoder_model, attention = attention).to(device)
    ...
    # train_loss_history = train_model(model = model, train_loader = simple_train, valid_loader = simple_valid, 
    #                                                      source_vocab = simple_source_vocab, target_vocab = simple_source_vocab,
    #                                                      criterion = criterion, optimizer = optimizer, num_epochs = num_epochs, learning_rate = learning_rate, 
    #                                                      print_ = True)
